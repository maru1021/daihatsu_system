from manufacturing.models import CastingMachine, CastingLine
...
class CastingMachineView(ManufacturingPermissionMixin, BasicTableView):
    title = '鋳造機'
    page_title = '鋳造機管理'
    ...
    delete_url = 'manufacturing:casting_machine_delete'
    excel_export_url = 'manufacturing:casting_machine_export_excel'
    excel_import_url = 'manufacturing:casting_machine_import_excel'
    ...
    def get_context_data(self, **kwargs):
        ...
        context['lines'] = CastingLine.get_active_names()
    ...
    def validate_data(self, data, pk=None):
        try:
            ...
            line = CastingLine.get_by_id(data.get('line_id', '')) if data.get('line_id', '') else None
...
class CastingMachineExcelView(ManufacturingPermissionMixin, ExcelOperationView):
    export_model = CastingMachine.objects.select_related('line').values(
        'id', 'line__name', 'name', 'active')
    import_model = CastingMachine
    excel_file_name = 'machine_master.xlsx'
    table_class =  CastingMachineView
    ...
    def validate_data(self, index, row, operation='追加'):
        try:
            ...
            if not line_name:
                ...
                line = CastingLine.get_by_name(line_name)
                if not line:
    ...
    def model_create(self, create_list, user):
        try:
            create_objects = [
                self.import_model(
                    line=CastingLine.cache_get_by_name(row['ライン名']),
                    name=row['設備名'],
                    active=row['アクティブ'] != '無効',
                    last_updated_user=user.username if user else None,
                )
    ...
    def model_update(self, update_list, update_models_dict, user):
        try:
            ...
            for row in update_list:
                ...
                obj.line = CastingLine.cache_get_by_name(row.get('ライン名'))
                obj.name = row['設備名']
