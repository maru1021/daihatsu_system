...
class CVTLineView(ManufacturingPermissionMixin, BasicTableView):
    title = 'CVTライン'
    page_title = 'CVTライン管理'
    crud_model = CVTLine
    table_model = CVTLine.objects.only(
        'id', 'name', 'occupancy_rate', 'changeover_time', 'active', 'last_updated_user'
    )
    form_dir = 'master/cvt_line'
    form_action_url = 'manufacturing:cvt_line_master'
    edit_url = 'manufacturing:cvt_line_edit'
    delete_url = 'manufacturing:cvt_line_delete'
    excel_export_url = 'manufacturing:cvt_line_export_excel'
    excel_import_url = 'manufacturing:cvt_line_import_excel'
    pdf_export_url = 'manufacturing:cvt_line_export_pdf'
...
class CVTLineExcelView(ManufacturingPermissionMixin, ExcelOperationView):
    export_model = CVTLine.objects.values('id', 'name', 'occupancy_rate', 'active')
    import_model = CVTLine
    excel_file_name = 'line_master.xlsx'
    table_class = CVTLineView
    ...
    def model_update(self, update_list, update_models_dict, user):
        try:
            ...
            self.import_model.objects.bulk_update(
                update_objects,
                ['name', 'occupancy_rate', 'active', 'last_updated_user']
            )
...
class CVTLinePDFView(ManufacturingPermissionMixin, PDFGenerator):
    title = 'ライン一覧'
    data = CVTLine.objects.all()
