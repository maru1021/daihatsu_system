from manufacturing.models import CVTMachine, CVTLine
...
class CVTMachineView(ManufacturingPermissionMixin, BasicTableView):
    title = 'CVT鋳造機'
    page_title = 'CVT鋳造機管理'
    crud_model = CVTMachine
    table_model = CVTMachine.objects.select_related('line').only(
        'id', 'line__name', 'name', 'active', 'last_updated_user'
    ).order_by('line__order', 'order')
    form_dir = 'master/cvt_machine'
    form_action_url = 'manufacturing:cvt_machine_master'
    edit_url = 'manufacturing:cvt_machine_edit'
    delete_url = 'manufacturing:cvt_machine_delete'
    excel_export_url = 'manufacturing:cvt_machine_export_excel'
    excel_import_url = 'manufacturing:cvt_machine_import_excel'
    ...
    def get_context_data(self, **kwargs):
        ...
        context['lines'] = CVTLine.get_active_names()
    ...
    def validate_data(self, data, pk=None):
        try:
            ...
            line = CVTLine.get_by_id(data.get('line_id', '')) if data.get('line_id', '') else None
...
class CVTMachineExcelView(ManufacturingPermissionMixin, ExcelOperationView):
    export_model = CVTMachine.objects.select_related('line').values(
        'id', 'line__name', 'name', 'active')
    import_model = CVTMachine
    excel_file_name = 'machine_master.xlsx'
    table_class =  CVTMachineView
    ...
    def extra_select(self, select_list):
        line_names = CVTLine.get_active_names(model=False)
    ...
    def validate_data(self, index, row, operation='追加'):
        try:
            ...
            if not line_name:
                ...
                line = CVTLine.get_by_name(line_name)
                if not line:
    ...
    def model_create(self, create_list, user):
        try:
            create_objects = [
                self.import_model(
                    line=CVTLine.cache_get_by_name(row['ライン名']),
                    name=row['設備名'],
                    active=row['アクティブ'] != '無効',
                    last_updated_user=user.username if user else None,
                )
    ...
    def model_update(self, update_list, update_models_dict, user):
        try:
            ...
            for row in update_list:
                ...
                obj.line = CVTLine.cache_get_by_name(row.get('ライン名'))
                obj.name = row['設備名']
