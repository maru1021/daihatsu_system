...
from management_room.models import CVTItem
from manufacturing.models import CVTLine

class CVTItemMasterView(ManagementRoomPermissionMixin, BasicTableView):
    title = 'CVT品番'
    page_title = 'CVT品番管理'
    crud_model = CVTItem
    table_model = CVTItem.objects.select_related('line').only(
        'id', 'line__name', 'name', 'optimal_inventory', 'molten_metal_usage', 'active', 'last_updated_user'
    )
    form_dir = 'master/cvt_item'
    form_action_url = 'management_room:cvt_item_master'
    edit_url = 'management_room:cvt_item_edit'
    delete_url = 'management_room:cvt_item_delete'
    ...
    def get_context_data(self, **kwargs):
        ...
        lines = CVTLine.objects.filter(active=True)
        context['lines'] = lines
        return context
