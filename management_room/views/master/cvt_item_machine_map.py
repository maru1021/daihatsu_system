...
from management_room.models import CVTItem, CVTItemMachineMap
from manufacturing.models import CVTLine, CVTMachine

class CVTItemMachineMapView(ManagementRoomPermissionMixin, BasicTableView):
    title = 'CVT品番-設備紐づけ'
    page_title = 'CVT品番-設備紐づけ'
    crud_model = CVTItemMachineMap
    table_model = CVTItemMachineMap.objects.select_related('line', 'machine', 'casting_item').only(
    ...
    form_dir = 'master/cvt_item_machine_map'
    form_action_url = 'management_room:cvt_item_machine_map'
    edit_url = 'management_room:cvt_item_machine_map_edit'
    delete_url = 'management_room:cvt_item_machine_map_delete'
    admin_table_header = ['CVTライン', '鋳造機', '鋳造品番', 'タクト', '良品率', 'アクティブ', '最終更新者', '操作']
    user_table_header = ['CVTライン', '鋳造機', '鋳造品番', 'タクト', '良品率', 'アクティブ', '最終更新者']
    search_fields = ['line__name', 'machine__name', 'casting_item__name']

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        lines = CVTLine.objects.filter(active=True).order_by('order')
        machines = CVTMachine.objects.select_related('line').filter(active=True).order_by('line__order', 'order')
        casting_items = CVTItem.objects.select_related('line').filter(active=True).order_by('line__order', 'order')
        context['lines'] = lines
    ...
    def update_model(self, model, data, user, files=None):
        try:
            model.line = CVTLine.objects.get(id=data.get('line_id', '').strip()) if data.get('line_id', '').strip() else None
            model.machine = CVTMachine.objects.get(id=data.get('machine_id', '').strip()) if data.get('machine_id', '').strip() else None
            model.casting_item = CVTItem.objects.get(id=data.get('casting_item_id', '').strip()) if data.get('casting_item_id', '').strip() else None
