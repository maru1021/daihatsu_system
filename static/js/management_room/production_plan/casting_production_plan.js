...
function checkItemChanges() {
    ...
    if (!isBlockLine) {
        ...
        vehicleSelectCache.forEach(select => {
            ...
            const nextSelect = getNextWorkingShiftSelect(dateIndex, shift, machineIndex);
            const prevSelect = getPrevWorkingShiftSelect(dateIndex, shift, machineIndex);
...
// 品番変更ハイライト用のヘルパー関数
// ========================================
// 次の稼働している直のselect要素を取得（casting専用、shared/casting.jsとは異なる実装）
function getNextWorkingShiftSelect(dateIndex, shift, machineIndex) {
    if (shift === 'day') {
        // 日勤の場合: 同じ日の夜勤を取得
...
// 前の稼働している直のselect要素を取得（casting専用、shared/casting.jsとは異なる実装）
function getPrevWorkingShiftSelect(dateIndex, shift, machineIndex) {
    if (shift === 'day') {
