...
// - utils.js: ユーティリティ関数（debounce, moveToNextShift, getMachineName等）
...
//
// 鋳造ライン固有の機能:
// - 金型管理（ヘッドのみ）: updateMoldCount, drawInheritanceArrows, updateReusableMolds等
// - ライン別処理: calculateProduction, applyItemChangeHighlights等
//
// ========================================
// パフォーマンス最適化の要点
// ========================================
...
// 定数
// ========================================
let REGULAR_TIME_DAY, REGULAR_TIME_NIGHT;
...
// グローバル変数（HTMLから渡される）
// ========================================
/* global itemData, previousMonthInventory, previousMonthProductionPlans, prevUsableMolds, isHeadLine, isBlockLine, isCoverLine, colorMap, setupColumnHover */
...
function getNextWorkingShiftWrapper(dateIndex, shift, machineIndex) {
    const caches = getCaches();
    return getNextWorkingShift(dateIndex, shift, machineIndex, caches);
}

function getPrevWorkingShiftWrapper(dateIndex, shift, machineIndex) {
    const caches = getCaches();
    return getPrevWorkingShift(dateIndex, shift, machineIndex, caches);
}

// ========================================
// セレクトボックス初期化（共通モジュール使用）
// ========================================
function initializeSelectColors() {
    initializeSelectColorsShared({
        ...
        onSelectChange: (select, dateIndex, shift, machineIndex) => {
            // 変更前の品番を取得
            const oldItem = select.getAttribute('data-vehicle') || '';
            ...
            const newItem = select.value || '';

            // 鋳造ライン: 金型カウント管理あり - 金型使用数を更新（生産台数・在庫も計算される）
...
function checkItemChanges() {
    ...
    if (!isBlockLine) {
        ...
        vehicleSelectCache.forEach(select => {
            ...
            // 土日や休日を跨いで稼働している直を取得（品番が入っているセルのみ）
            const nextSelect = getNextWorkingShiftSelect(dateIndex, shift, machineIndex, vehicleSelectCache);
            const prevSelect = getPrevWorkingShiftSelect(dateIndex, shift, machineIndex, vehicleSelectCache);
...
// 品番変更ハイライト用のヘルパー関数
// ========================================
// getNextWorkingShiftSelectとgetPrevWorkingShiftSelectは
// shared/casting/utils.jsに移動されました（鋳造・CVT共通）
...
function setupEventListeners() {
    setupEventListenersShared({
        ...
        rebuildInventoryCache: () => {
            inventoryElementCache = null;  // キャッシュを無効化
        },
        addMoldCountListeners: isHeadLine,  // ヘッドラインのみ金型カウントリスナーを追加
        onMoldCountDoubleClick: toggleMoldCountManualBlock,
        onMoldCountRightClick: showMoldCountEditModal
    });
}
...
function saveProductionPlan() {
    saveProductionPlanShared({
        includeMoldCount: isHeadLine,  // ヘッドラインのみ金型カウント管理あり
        getMoldCountData: (container, shift, dateIndex, machineIndex) => {
            // ヘッドラインのみ金型データを取得
            if (!isHeadLine) return 0;
            ...
            const isManualBlock = moldCountDisplay && moldCountDisplay.getAttribute('data-manual-block') === 'true';
            return isManualBlock ? -1 : moldCount;
        },
        getAdditionalData: () => {
            // Casting: usable_molds_data を追加
            let usableMoldsData;
            if (window.autoGeneratedUnusedMolds && window.autoGeneratedUnusedMolds.length > 0) {
                ...
                usableMoldsData = window.autoGeneratedUnusedMolds.map(mold => ({
                    ...
                    machine_name: mold.machine_name || '',
                    end_of_month: mold.end_of_month || false
                }));
            } else {
                // 手動操作の場合は再利用可能金型リストを収集
                usableMoldsData = collectUsableMoldsData();
            }
            return { usable_molds_data: usableMoldsData };
        },
        domConstantCache,
        getCookie,
...
// 旧saveProductionPlan関数本体は共通モジュールに移行済みのため削除
// ========================================
// 自動生産計画（共通モジュール使用）
// ========================================
...
async function applyAutoProductionPlan(planData) {
    await applyAutoProductionPlanShared(planData, {
        ...
        recalculateAllInventory: recalculateAllInventoryWrapper,
        applyItemChangeHighlights,
        onPlanApplied: async (allDateIndices) => {
            // Casting固有: 前月金型データクリアと金型カウント再計算
            prevMonthMoldsOriginal = [];
            prevMonthMoldsStatus = [];
            cellToPrevMonthMoldIndex = {};

            // ヘッドラインの場合のみ金型カウントを再計算
            if (isHeadLine) {
                const dateCount = domConstantCache.dateCount;
                const totalMachines = domConstantCache.totalMachines;
                for (let d = 0; d < dateCount; d++) {
...
async function initialize() {
    ...
    // ========================================
    buildDOMCacheWrapper({ includeMoldCount: true });  // DOM要素をキャッシュ（鋳造は金型カウントあり）
